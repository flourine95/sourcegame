...
public class ChangeMapService {
        ...
        try {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                case ConstMap.CHANGE_CAPSULE:
                    ...
                    if (pl.isBeQuynh) {
                        Service.gI().sendThongBao(pl, "Đang hộ tống Bunma không thể Capsule");
                        return;
                    }
    ...
    public void openZoneUI(Player pl) {
        if (pl.isBeQuynh) {
            Service.gI().sendThongBao(pl, "Đang Hộ tống Bunma không thể đổi khu");
            return;
        }
        if (pl.zone == null) {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 10000)) {
            ...
            Service.gI().sendThongBao(pl, "Không thể đổi khu vực lúc này, vui lòng đợi "
    ...
    public void changeMapWaypoint(Player player) {
        ...
        if (zoneJoin != null) {
            ...
            if(player.isBeQuynh && !Util.canDoWithTime(player.lastTimeHoTong, 10000)){
                resetPoint(player);
                Service.gI().sendThongBao(player, "Đang Hộ tống Bunma không được chuyển map quá nhanh !!");
                return;
            }
            if(player.isBeQuynh){
...
//        }
        ...
        if (player != null && player.isPl() && !player.isAdmin()) {
            switch (zoneJoin.map.mapId) {
                ...
                case 102, 92, 93, 94, 96, 97, 98, 99, 100, 104 -> {
                ...
                case 142 -> {
                    if (player.getSession().player.nPoint.power <= 130000000000L) {
                        Service.gI().sendThongBao(player, "Yêu cầu sức mạnh trên 130 tỷ");
                        return null;
                    }
                }
                case 144 -> {
                    if (!player.inventory.haveOption(player.inventory.itemsBody, 5, 33)) {
                        Service.gI().sendThongBao(player, "Yêu cầu có cải trang dịch chuyển tức thời");
                        return null;
                    }
                }
                case 169, 146 -> {
    ...
    private void checkJoinSpecialMap(Player player) {
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                case 85, 86, 87, 88, 89, 90, 91 -> BlackBallWar.gI().joinMapBlackBallWar(player);
                case 114, 115, 117, 118, 119, 120 -> MapMaBu.gI().joinMapMabu(player);
            }
        }
    }
